...
def save_snapshot(inputs, sample, debug_dir, epoch, targets):
    """
    This function saves debug/monitoring visualization 
    """
    fig, axes = plt.subplots(1, 3)
    fig.suptitle(f"Epoch {epoch + 1}", fontsize=14)

    axes[0].imshow(inputs[0, 1].cpu(), cmap="gray", vmin=0, vmax=1)
    axes[0].set_title("condition")
    axes[0].axis("off")

    axes[1].imshow(sample[0, 0].cpu(), cmap="gray", vmin=0, vmax=1)
    axes[1].set_title("generated")
    axes[1].axis("off")

    axes[2].imshow(targets[0, 0].cpu(), cmap="gray", vmin=0, vmax=1)
...
def gen_snapshot(targets, scheduler, inputs, unet, debug_dir, epoch, device):
    """
    ...
    save_snapshot(inputs, sample, debug_dir, epoch, targets)
    return sample
...
def load_checkpoint(unet, optimizer, path, scaler=None, lr_scheduler=None):
    ...
    if os.path.exists(path):
        ...
        train_losses = checkpoint.get("train_losses", checkpoint.get("epoch_losses", []))
...
def train_diffusion(device, tl, vl, diffusion_ckpt_path, last_ckpt_path):
    ...
    debug_dir = os.path.join(config.RESULTS_DIR, "models_control/debug_snapshots")
    ...
    fixed_targets = fixed_batch[3][:1].to(device)
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        for step, batch in enumerate(tl):
            ...
            targets = targets.to(device)  # [B, 1, H, W] # post-contrast image
        ...
        if (epoch + 1) % val_interval == 0:
            ...
            with torch.no_grad():
                ...
                if do_snapshot:
                    sample = gen_snapshot(fixed_targets, scheduler, fixed_inputs, unet, debug_dir, epoch, device)

                    # calculate visual evaluation metrics on the denoised debug slice
                    snapshot_mae = torch.mean(torch.abs(sample - fixed_targets)).item()
                    mse = torch.mean((sample - fixed_targets) ** 2).item()
                    psnr = 20 * math.log10(1.0 / math.sqrt(mse)) if mse > 0 else 100.0
                    ssim = ssim_metric(sample, fixed_targets).mean().item()
                    ssim_metric.reset()
...
if __name__ == "__main__":
    config.CURRENT_TRAINING = "control"
    os.makedirs(config.RESULTS_DIR, exist_ok=True)

    # path to best training state
    diffusion_ckpt_path = os.path.join(config.RESULTS_DIR, "models_control/best_diffusion.pth")

    # path to resumable (last) training state
    last_ckpt_path = os.path.join(config.RESULTS_DIR, "models_control/diffusion_checkpoint.pth")
